...
class PostViewsTestCase(TestCase):
    ...
    def test_image_make_first_POST(self):
        self.client.force_login(self.user_ostin)

        self.assertIsNone(Post.objects.get(pk=self.post_ostin.pk).first_image)

        response = self.client.post(reverse('image_update', kwargs={'pk': self.post_ostin.pk}),
                                    {'image_pk': self.post_ostin.image_set.first().pk})
